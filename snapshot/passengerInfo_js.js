...
var intervalProcess; (function(a) {
    ...
    function p() {
        ...
        for (var z = 0; z < limit_tickets.length; z++) {
            var D = limit_tickets[z];
            E.push(D.ticket_type);
            if ((ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) && ticket_submit_order.ticket_type.child != D.ticket_type) {
                if (jQuery.inArray(D.id_no, B) < 0) {
                    B.push(D.id_no)
                } else {
                    ...
                    l("出票失败", true, "互联网售票实行实名制：证件号<span style='color:red'><i><b>" + D.id_no + "</b></i></span>只能购买" + A + "车次" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.station_train_code + "的一张车票！", true, "warn");
                    return false
                }
            } else {
                if ((ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) && D.save_status != "" && ticket_submit_order.ticket_type.child != D.ticket_type) {
                    if (jQuery.inArray(D.id_no, B) < 0) {
                        B.push(D.id_no)
                    } else {
                        var A = ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(0, 4) + "年" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(4, 2) + "月" + ticketInfoForPassengerForm.queryLeftTicketRequestDTO.train_date.substr(6, 2) + "日";
            ...
            if (D.save_status != "") {
                ...
                if (ticket_submit_order.tour_flag.gc == ticketInfoForPassengerForm.tour_flag) {
                    if (v.length > 0) {
                        if (jQuery.inArray(D.seat_type, v) < 0) {
                            l("出票失败", true, (CHANGETSFLAG == "Y" ? "变更到站": "改签") + "时，必须选择相同席别", true, "warn");
                            return false
                        }
                    }
                    v.push(D.seat_type);
                    if (w.indexOf(D.seat_type) > -1) {
                        C.push(D.seat_type)
                    }
                    if (C.length > 1) {
            ...
            if (D.ticket_type == ticket_submit_order.ticket_type.disability) {
                var x = id_type_code;
                if (x != ticket_submit_order.passenger_card_type.two) {
                    ...
                    if (D.id_type != ticket_submit_order.passenger_card_type.two) {
            ...
            if (ticket_submit_order.tour_flag.wc == ticketInfoForPassengerForm.tour_flag || ticket_submit_order.tour_flag.dc == ticketInfoForPassengerForm.tour_flag) {
                if (D.ticket_type == ticket_submit_order.ticket_type.student) {
                    if (D.passenger_type != ticket_submit_order.passenger_type.student) {
...
var dwLimitErrorStStr = "整包购买，所有乘车人请选择相同的席别(动卧或高级动卧)"; (function() {
    ...
    function ao(aA) {
        ...
        $("#dj_passenger_id li").click(function() {
            var aB = $(this).find("input");
            excuteClickEditPassenger(aB)
        })
    }
    function U(az) {
        if (az) {
                        ...
                        if (aA == ticket_submit_order.passenger_card_type.hongkong_macau) {
                            if ($.trim(aC) != "") {
                                if (!/^[HMhm]{1}[0-9]{10}$/.test(aC) && !/^[HMhm]{1}[0-9]{8}$/.test(aC)) {
                                    $("#error_update_tr").show();
                                    $("#error_for_update_nameandidno").html("非法的港澳通行证号码").show();
                                    return false
                                }
                            } else {
                                $("#error_update_tr").show();
                            ...
                            if (aA == ticket_submit_order.passenger_card_type.taiwan) {
                                if ($.trim(aC) != "") {
                                    if (!/^[0-9]{8}$/.test(aC) && !/^[0-9]{10}$/.test(aC)) {
                                        $("#error_update_tr").show();
                                        $("#error_for_update_nameandidno").html("非法的台湾通行证号码").show();
                                        return false
                                    }
                                } else {
                                    $("#error_update_tr").show();
    ...
    renderTickInfo = function(aG, aC) {
        var aK = $("#ticketInfo_id tr");
        var aJ = aK.length;
        var aB = aG.length;
        var aA = null;
        var az = $("#seatType_1").val();
        var aF = aG[aB - 1].seatTypes;
        var aL = false;
        if (aJ >= 2) {
            var aI = aK.eq(aJ - 2).attr("id");
            aA = $("#seatType_" + aI.substr(6)).val()
        }
        if (aA) {
        ...
        upadateSavePassengerInfo();
        var aN = $("#ticketInfoTemplate").html().replace("<!--", "").replace("-->", "");
        $.templates({
            leftTableTemplate: aN
        });
        aG.IsStudentDate = IsStudentDate;
        $("#ticketInfo_id").html($.render.leftTableTemplate(aG));
                    ...
                    if (aA == ticket_submit_order.passenger_card_type.work) {
                        ...
                        if (aA == ticket_submit_order.passenger_card_type.hongkong_macau) {
                            if ($.trim(aC) != "") {
                                if (!/^[HMhm]{1}[0-9]{10}$/.test(aC) && !/^[HMhm]{1}[0-9]{8}$/.test(aC)) {
                                    $("#error_tr").show();
                                    $("#error_for_nameandidno").html("非法的港澳通行证号码").show();
                                    return false
                                }
                            } else {
                                $("#error_tr").show();
                            ...
                            if (aA == ticket_submit_order.passenger_card_type.taiwan) {
                                if ($.trim(aC) != "") {
                                    if (!/^[0-9]{8}$/.test(aC) && !/^[0-9]{10}$/.test(aC)) {
                                        $("#error_tr").show();
                                        $("#error_for_nameandidno").html("非法的台湾通行证号码").show();
                                        return false
                                    }
                                } else {
                                    $("#error_tr").show();
